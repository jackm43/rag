...
// migration (workers/public/dev-proxy/migrations); Better Auth never introspects
...
export const resolveDiscordSubject = async (
  ...
  return {
    ...
    // Present because session.additionalFields.accessSub is read back on every
    // getSession; undefined only for a legacy session minted before binding.
    accessSub: session.session.accessSub ?? undefined,
