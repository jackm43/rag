...
// The gateway's HTTP surface is CONSTRUCTED from openapi.yaml (via the
// generated route table): unknown paths 404, undeclared methods 405 with the
// spec's Allow set, and each operation's security scheme selects the ingress
// guard that must pass before its handler runs. Handlers are keyed by
// operationId; a spec operation without a handler fails construction, so the
// worker cannot deploy with an unimplemented route.
  ...
  for (const routes of Object.values(GATEWAY_ROUTES)) {
    for (const route of routes) {
      if (!handlers[route.operationId]) {
        throw new Error(`openapi.yaml operation "${route.operationId}" has no handler`);
      }
    }
  }
