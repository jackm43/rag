...
import { WEBHOOKS_MANIFEST } from "./manifest";
...
//
// Routing is a small hand-rolled matcher like the dev-proxy's: the gateway's
// generated route table (scripts/generate-gateway-routes.ts) keys routes by
// EXACT pathname and cannot express the templated /{provider}/{id} path, so
// the openapi.yaml here is documentation-first, not codegen input.
...
const handleWebhook = async (
  ...
  try {
    const result = await connectorsClient(
      env,
      serviceClients(env).webhooksToConnectors,
      subject,
  ...
  try {
    await serviceClients(env).webhooksToWorkflows.call({
      transport: "queue",
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const match = new URL(request.url).pathname.match(WEBHOOK_PATH_PATTERN);
    if (!match) {
      return notFound();
    }
