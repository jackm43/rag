...
import { createClient, createHopIntent, ensureRegistered, SYSTEM_SUBJECT } from "../../../../../packages/auth";
import {
  encodeApplicationRequestEnvelope,
  MAX_APPLICATION_BODY_BYTES,
  type PreparedApplicationRequest,
} from "../../../../../packages/contracts";
import type { Env } from "../../../../../packages/contracts/types";
import { envelopeSha256 } from "../../../../../packages/identity";
...
export class ApplicationMiddleware extends WorkerEntrypoint<Env> {
  async prepare(request: Request): Promise<PreparedApplicationRequest> {
    ...
    try {
      ...
      }).to("application-service", { transportTrust: "application" }).prepare(envelope, {
