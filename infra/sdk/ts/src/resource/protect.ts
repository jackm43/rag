...
import { clientInstance } from "../auth/proxy";
  ...
  for (const method of service.methods) {
    ...
    const authorize = async (context: HandlerContext) => {
      ...
      if (policy.allow && !policy.allow(identity)) {
        logger.warn("request_denied", { method: methodName, principal, reason: "policy" });
        throw new ConnectError("forbidden", Code.PermissionDenied);
      }
      if (scope && !hasScope(identity, scope)) {
        logger.warn("request_denied", { method: methodName, principal, reason: "scope", scope });
