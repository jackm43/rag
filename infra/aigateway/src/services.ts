...
import { ChatService } from "../../applications/aigateway/server/aigateway/v1/aigateway_pb";
import {
  anyAuthenticator,
  errorMessage,
  logger,
...
export const registerAiGatewayServices = (router: ConnectRouter, env: Env, tracer: Tracer) => {
  ...
  const verify = {
    ...
    jwksFetch: env.AUTH_GATEWAY ? (input: RequestInfo | URL, init?: RequestInit) => env.AUTH_GATEWAY!.fetch(input, init) : undefined,
  };
