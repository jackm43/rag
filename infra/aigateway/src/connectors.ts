import {
  createLeaderboardServiceClient,
  type Connection as RagbotConnection,
} from "../../applications/ragbot/service";
import {
  errorMessage,
  logger,
...
const ragbotConnection = (env: Env): RagbotConnection | null => {
  const credential = serviceCredentialFromEnv(env);
  if (!credential || !env.RAGBOT_ENDPOINT) {
    return null;
  }
  return {
    ...
    gatewayFetch: env.AUTH_GATEWAY
      ...
      : undefined,
    fetch: env.RAGBOT
      ? (input: RequestInfo | URL, init?: RequestInit) => env.RAGBOT!.fetch(input, init)
      : undefined,
  };
};
