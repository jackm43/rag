...
type ApplicationRow = {
  ...
  provider_oauth_client_id: string;
  provider_oauth_scopes: string;
  created_at: number;
  updated_at: number;
};
...
const fromRow = (row: ApplicationRow): RegisteredApplication => ({
  ...
  providerOauthClientId: row.provider_oauth_client_id ?? "",
  providerOauthScopes: JSON.parse(row.provider_oauth_scopes || "[]") as string[],
...
export const upsertApplication = async (
  ...
  await env.DB.prepare(
    `INSERT INTO idp_applications (name, audience, endpoint, description, resources, provider, trust_boundary, access, impersonation_access_client_id, provider_oauth_client_id, provider_oauth_scopes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(name) DO UPDATE SET
       ...
       END,
       provider_oauth_client_id = CASE
         WHEN excluded.provider_oauth_client_id != '' THEN excluded.provider_oauth_client_id
         ELSE idp_applications.provider_oauth_client_id
       END,
       provider_oauth_scopes = CASE
         WHEN excluded.provider_oauth_scopes != '[]' THEN excluded.provider_oauth_scopes
         ELSE idp_applications.provider_oauth_scopes
       END,
    ...
    .bind(
      ...
      app.providerOauthClientId ?? "",
      JSON.stringify(app.providerOauthScopes ?? []),
    )
    .run();
...
export const serviceClientApplication = async (
  env: Env,
  clientId: string,
): Promise<string | null> => {
  const row = await env.DB.prepare(
    "SELECT application FROM idp_service_clients WHERE client_id = ?",
  )
    .bind(clientId)
    .first<{ application: string }>();
