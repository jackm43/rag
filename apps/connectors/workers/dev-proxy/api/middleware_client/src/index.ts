...
import { createClient, createHopIntent, ensureRegistered } from "@rag/service-kit";
import { cloudflareAccessGuard } from "@rag/ingress/cf-access";
import { connectorsClient } from "../../../../../lib";
import { encodeDevProxyCommandEnvelope } from "../../../../../contracts";
import type { ConnectorResult, DevProxyCommandJob, Env } from "../../../../../contracts";
import { errorMessage, logger } from "@rag/logger";
import type { components } from "../../../../../devproxy-client/api-types";
import {
  AuthUnconfiguredError,
  createAuth,
  resolveDiscordSubject,
  type ResolvedSubject,
} from "@rag/ingress/better-auth";
...
const handleCommand = async (request: Request, env: Env, accessSub: string): Promise<Response> => {
  ...
  try {
    ...
    }).to("gateway", { transportTrust: "trusted" }).prepare(envelope, {
