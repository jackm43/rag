...
import { SIGNING_KEY_JWKS, signedServiceMessage } from "../../helpers";
...
test("a token signed by the wrong key for a real issuer is denied at the server", async () => {
  ...
  try {
    ...
    // Verification applies to http (the externally reachable transport);
    // binding/queue are capability-trusted and read claims without a signature.
    const denied = await server.receive(encodeServiceMessage(envelope, forgedToken), decodeReplyJobEnvelope, "http");
...
test("a valid token replayed against different envelope bytes is denied at the server", async () => {
  ...
  try {
    ...
    const server = createServiceServer({ self: "responder", expectedIssuers: ["workflows"] });
    // Envelope-binding is enforced by signature verification, i.e. over http.
    const denied = await server.receive(replayed, decodeReplyJobEnvelope, "http");
    assert.equal(denied, null);
    const denial = warnings.lines.find((line) => line.message === "service_denied");
